...
BUFFER_PATH = Path("Buffer/raw_buffer.csv")
...
class SVD_method():
    buffer_dir = Path("Buffer/svd_results")
    buffer_model_path = [
    ...
    def train(self, data:pd.DataFrame):
        ...
        model["model"] = individual_train(data.dropna(subset="book_rating"), "book_rating")
    ...
    def prepare_data(self, unprepared_data:pd.DataFrame, user_threshold=3):
        data = unprepared_data.copy()
...
class Statisical_method():
    buffer_dir = Path("Buffer/statistical_results")
    ...
    def update(self, data:pd.DataFrame):
        try:
            data = self.prepare_data(data)
            print("Update Statistical Method...")
    ...
    def update_buffer(self, df_recommend):
        ...
        print("Statistical Method Update Successful")
        
    def prepare_data(self, unprepared_data:pd.DataFrame):
        data = unprepared_data.copy().dropna(subset="book_rating")
        return data
    ...
    def calculate_recommendation(self):
        ...
        def cal_controversial(df):
            df = df.groupby('book_id')['book_rating'].agg(['count', 'std'])
            df = df[df['count'] > 50].sort_values("std", ascending=False)
            df = df.rename(columns={"std":"standard_deviation"})
            df = df.round(decimals=2)
            df.reset_index(drop=False, inplace = True)
            return df
